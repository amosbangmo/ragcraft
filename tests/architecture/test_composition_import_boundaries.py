...
COMPOSITION_PY = sorted((REPO_ROOT / "src" / "composition").glob("*.py"))
...
def _forbidden_transport_import_line(line: str) -> bool:
    ...
    if s.startswith("#") or not s:
        return False
    return s.startswith("from src.frontend_gateway") or s.startswith("import src.frontend_gateway")
...
def test_composition_modules_do_not_import_frontend_gateway(path: Path) -> None:
    ...
    assert not hits, (
        f"{path.relative_to(REPO_ROOT)} must not import src.frontend_gateway "
