...
def test_composition_modules_do_not_import_services_package(path: Path) -> None:
    ...
    assert not hits, (
        ...
        f"(use ports + infrastructure adapters; Streamlit-session test composition uses "
        f"support.backend_container.build_streamlit_session_aware_backend_container_for_tests):\n"
        + "\n".join(hits)
    )
