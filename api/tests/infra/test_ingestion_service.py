...
if "infrastructure.config.config" not in sys.modules:
    # Minimal config used by the service constructor in tests.
    config_module = types.ModuleType("infrastructure.config.config")
    config_module.INGESTION_CONFIG = types.SimpleNamespace(
        extraction_max_text_chars_per_asset=2500,
        summary_max_input_chars=4000,
    )
    sys.modules["infrastructure.config.config"] = config_module

if "infrastructure.rag.ingestion.loader" not in sys.modules:
    # Stub external ingestion modules to avoid importing heavy dependencies.
    loader_module = types.ModuleType("infrastructure.rag.ingestion.loader")
    loader_module.save_uploaded_file = lambda uploaded_file, path: path
    sys.modules["infrastructure.rag.ingestion.loader"] = loader_module

if "infrastructure.rag.ingestion.unstructured_extractor" not in sys.modules:
    extractor_module = types.ModuleType("infrastructure.rag.ingestion.unstructured_extractor")
    extractor_module.extract_elements = lambda file_path, source_file: []
    sys.modules["infrastructure.rag.ingestion.unstructured_extractor"] = extractor_module

if "infrastructure.rag.ingestion.summarizer" not in sys.modules:
    summarizer_module = types.ModuleType("infrastructure.rag.ingestion.summarizer")
...
class TestIngestionService(unittest.TestCase):
    ...
    @patch("infrastructure.rag.ingestion_service.extract_elements")
