...
Used by the HTTP layer (via :mod:`apps.api.schemas.serialization`) so ``apps/api`` does not
import LangChain or other retrieval-framework types. **Backward compatible:** output shapes match
historical ``page_content`` + ``metadata`` dicts for document-like objects.
"""
