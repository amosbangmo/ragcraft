...
class FailureAnalysisService:
    """
    Rule-based failure taxonomy over benchmark rows (dict-shaped, as from
    :meth:`~src.domain.benchmark_result.BenchmarkRow.to_dict`).

    Missing metrics are ignored for that rule (no crash). Rows may receive
    multiple labels.
    """
    ...
    def _classify_row(self, row: dict[str, Any]) -> tuple[list[str], bool]:
        ...
        cit_prec = _coerce_float(row.get("citation_doc_id_precision"))
        ...
        # Gold answer mismatch (token F1) when no other grounding signal fired yet.
        if (
            has_gold
